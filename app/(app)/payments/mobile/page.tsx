    ...
    <PageTransition>
      <div className="min-h-screen bg-[#f8f9fa] dark:bg-gray-900">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
            <AmountInput value={amount} onChange={setAmount} currency={defaultAccount?.currency || "USD"} />
          </div>

          <div className="flex justify-between text-sm">
            <div className="text-gray-600 dark:text-gray-400">
              Your balance: {formatCurrency(defaultAccount?.balance || 4863.27, defaultAccount?.currency || "USD")}
            </div>
            <div className="text-gray-600 dark:text-gray-400">No fees</div>
          </div>

          <TransactionSummary amount={amount || "0"} currency={defaultAccount?.currency || "USD"} />

          <Button
            type="submit"
            className="w-full bg-[#0a0b25] hover:bg-[#1a1b35] h-12"
            disabled={loading || !amount || !phoneNumber || phoneNumber === "+17 "}
          >
            {loading ? (
